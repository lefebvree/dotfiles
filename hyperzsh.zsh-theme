# The prompt
PROMPT='$(_user_host)$(_python_venv)%{$fg[cyan]%}%2c $(git_prompt_info)%{$reset_color%}$(_git_time_since_commit)$(git_prompt_status)$(_task_number)${_return_status}→ '
...
function _task_number() {
	if [ -f .todo ]
	then
		echo "%{$fg[yellow]%}$(wc -l < .todo)%{$reset_color%} "
	fi
}
...
ZSH_THEME_GIT_PROMPT_MODIFIED="%{$fg[yellow]%}ahead↗ "
